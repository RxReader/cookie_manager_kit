#import "FakeCookieManagerPlugin.h"
#import <WebKit/WebKit.h>
#import <time.h>
#import <xlocale.h>

@implementation FakeCookieManagerPlugin
+ (void)registerWithRegistrar:(NSObject<FlutterPluginRegistrar>*)registrar {
    FlutterMethodChannel* channel = [FlutterMethodChannel
                                     methodChannelWithName:@"v7lin.github.io/fake_cookie_manager"
                                     binaryMessenger:[registrar messenger]];
    FakeCookieManagerPlugin* instance = [[FakeCookieManagerPlugin alloc] init];
...
- (void)saveCookies:(FlutterMethodCall*)call result:(FlutterResult)result {
    ...
    if (cookieStrs != nil && cookieStrs.count > 0) {
        for (NSString * cookieStr in cookieStrs) {
            ...
            NSArray<NSHTTPCookie *> * cookies = [NSHTTPCookie cookiesWithResponseHeaderFields:headerFields forURL:[NSURL URLWithString:url]];
            if (@available(iOS 11.0, *)) {
                for (NSHTTPCookie * cookie in cookies) {
                    [[[WKWebsiteDataStore defaultDataStore] httpCookieStore] setCookie:cookie completionHandler:nil];
                }
            } else {
                [[NSHTTPCookieStorage sharedHTTPCookieStorage] setCookies:cookies forURL:[NSURL URLWithString:url] mainDocumentURL:nil];
            }
        }
    }
    result(nil);
...
- (BOOL) domainMatch:(NSString *)urlHost domain:(NSString *)domain {
    ...
    if ([urlHost hasSuffix:domain] && [urlHost characterAtIndex:(urlHost.length - domain.length - 1)] == '.' && ![self verifyAsIpAddress:urlHost]) {
...
- (void)removeAllCookies:(FlutterMethodCall*)call result:(FlutterResult)result {
    if (@available(iOS 11.0, *)) {
